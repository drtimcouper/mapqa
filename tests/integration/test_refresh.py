...
def test_ds_duplicate_name():
    try:
        ...
        # tidy up the created "expected..." directory
        shutil.rmtree(os.path.join(DATA_DIR, 'expected_duplicate_name'), ignore_errors=True)
...
def test_ds_simple():
    try:
        with patch.object(refresh, 'get_content_from_url') as get_content:
            ...
            # tidy up the created "expected..." directory
            shutil.rmtree(os.path.join(DATA_DIR, 'expected_simple'), ignore_errors=True)
